...
| **C2** | Self-heal breadth plus auto resource-scaling | M2 to M3 (resource-aware + missing-index `.fai` slices shipped; rest of index family + wider catalog + peak-RSS pending) | Bounded OOM/walltime retries up to an absolute ceiling with honest `gave_up_at_ceiling` give-up; a missing FASTA `.fai` is now built (`samtools faidx`) and retried, with honest `index_unresolvable`/`index_build_failed` give-up; rest of the index family (`.bai`, `.tbi`/`.csi`, `.dict`, STAR/BWA) and the wider catalog (reference/build mismatch, format, pin conflict) deferred |
