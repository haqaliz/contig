...
| Repair success-rate analytics | Across all runs: auto-healed vs paused vs gave-up, by failure class | Built data (the `patch_applied` slice supplied the missing field — proposed vs applied was previously indistinguishable, so any such analytic would have over-counted); cross-run aggregation still to build | M |
