...
| Repair success-rate analytics | Across all runs: auto-healed vs paused vs gave-up, by failure class | **Shipped (v0.57.0; attendance corrected Unreleased)** — `contig repair-stats`: per-step outcome-family and failure-class counts plus a per-run unattended-completion rate that states its own denominator. Three-state on both under-determined axes: `patch_applied` key presence (read / legacy-derived / unknown, since `models.py:322` defaults `False` and pydantic cannot distinguish an absent key) and attendance (the gated literals resolve against the run's own `--auto-approve` fact, now persisted on `LaunchManifest.auto_approve`; `attendance_unknown` means only that a given run did not record it). Zero-event and attendance-unknown runs are excluded from both sides of the rate and the exclusions are printed. Read-only over existing bundles — no new instrumentation | M |
...
| RO-Crate / interoperable export | Map the bundle to RO-Crate so provenance interoperates beyond Contig | Shipped 2026-06-23 (contig export --rocrate: an RO-Crate ro-crate-metadata.json with the run as a Dataset, the pipeline as a SoftwareApplication, inputs/outputs as Files with checksums; dashboard download button). The reference genome and annotation-tool provenance are now in the crate too (Unreleased, `rocrate-reference-identity`): a `#reference` Dataset plus `#annotation-{n}` entities linked via `mentions`, and a `@context` term map that also fixes a pre-existing defect where `sha256` was dropped by JSON-LD expansion on every crate. | L |
...
| **C5** | Reference and input-data integrity | M5 (reference-identity **capture** slice shipped; **RO-Crate projection of the identity shipped (Unreleased)**; mismatch detector (incl. known-sites/GTF-version consistency) + annotation/GTF version resolution pending) | Record reference identity into provenance — explicit FASTA/GTF `sha256` or the iGenomes key (never a fabricated hash), rendered in `contig methods` and the provenance panel; deepens reproduce and seeds C2's mismatch repair. Catching wrong-reference runs at pre-flight is the next slice |
