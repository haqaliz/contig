...
| **C2** | Self-heal breadth plus auto resource-scaling | M2 to M3 (resource-aware slice shipped; wider catalog + peak-RSS pending) | Bounded OOM/walltime retries up to an absolute ceiling with honest `gave_up_at_ceiling` give-up; wider failure catalog (index, reference/build mismatch, format, pin conflict) deferred |
