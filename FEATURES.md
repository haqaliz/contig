...
| **C4** | New assay, depth-first: somatic variant calling | **Shipped v0.13.0** (intake→launch→verify slice) | A somatic tumor–normal assay end to end on nf-core/sarek: explicit persisted `--assay` (fixes the shared-pipeline collision), sarek tumor/normal sample-sheet pre-flight, `--tools strelka,mutect2` launch seam, structural manifest + methods label; VAF/PON plausibility and the Strelka2-vs-Mutect2 concordance hook deferred |
