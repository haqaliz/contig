...
- **`contig infer-locators <repo> <claims.json> --out <out.json> [--metrics
  ...
  matcher: it reads an existing `extract-claims` draft through the **unchanged**
  `load_claims`, sweeps a **local repo directory** (the shipped
  `classify_repo_argument` — local-only by design, no network, no fetching; a
  URL, DOI, or leading-`-` argument gets the classifier's honest refusal,
  exit 1), matches via the shipped `sweep_repo` + `match_claims`, and writes
  two files. The updated claims file: a bound claim gains locator keys
  (`from`+`path`, or `from`+`column`+`row`+`header` for a table locator —
  never `delimiter`, which `load_claims` re-derives); every non-bound claim
  keeps exactly `{id, value, tolerance}` — **semantically identical** to the
  input, parsed and asserted, not byte-compared. The review sidecar
  (`<out>.review.md`) carries the matcher's per-claim lines verbatim, a
  `Sweep skips (N)` section naming each skipped artifact and its reason (the
  sweep's never-silent-loss guarantee surfaced at the CLI), and the
  matching-mode note. The write path carries the extract-claims discipline:
  temp via `mkstemp` in the `--out` parent, the **unchanged `load_claims`
  round-trip** before any commit (a `ClaimsError` there is an internal bug —
  exit 1, nothing written), atomic `os.replace`, temp removed either way.
  Guards in the pinned order: `--out` == the claims input is refused (it would
  clobber the draft), an existing `--out` needs `--force`, a missing out parent
  is refused, and **nothing is written on any input failure**; `--dry-run` does
  the full read+sweep+match, prints the summary and sidecar, and writes
  nothing — not even the temp file.
  - **The metric-words wiring, and why it is a new flag.** The dig found that
    metric words do NOT survive to disk: the draft is `{id, value, tolerance}`
    only, the extract sidecar renders id/value/unit/origin/source_text (not
    `metric`), and `load_claims`' `Claim` has no metric field — and modifying
    `extract-claims` is out of scope by design. So `--metrics <json>` (claim
    id → metric-word list) threads the M10 mapping into the command; it is
    **optional** — absent, the command runs value-only (semantic filter off)
    and says so honestly. A stale map is never silent: ids regenerated by
    extraction (PRD R4) fall back to value-only in the matcher by design, and
    the echo names the matched/unmatched claim-id counts.
  - **The manual smoke, recorded (integrator-run, real CLI).** On the gate
    repo (`ritvikK05/rnaseq-reanalysis-htt`) with the gate's 20-claim reviewed
    draft and the gate's metric map: **6 bound / 7 ambiguous /
    3 refused_low_information / 4 no_candidates, 0 wrong** — every bound
    locator re-resolved through the real resolvers to exactly the claim value
    (2252, 68.2, 2326, 2226, 2826, 62.6), the out file round-trips the
    unchanged `load_claims` (20 claims), and the numbers are identical to the
    M10 evidence gate's — end to end through the real command.
  - **Read honestly.** Push, not demand-pull: the smoke is one repo, a
    moving-`main` shallow clone, with the author-of-record's draft and metric
    map; zero binds is an honest valid result (out still written, counts
    echoed — never a failure); `extract-claims` untouched; no
    `models.py`/verdict/bundle/signature change; no new dependency; guard
    baselines unmoved. The paper → verdict chain's last manual step shrinks to
    reviewing proposals: `extract-claims` → review (metric words) →
    `infer-locators` → review the sidecar → `contig reproduce` (unchanged; the
    freshness guard still decides every verdict). Spec and plan under
    `docs/planning/reproduce-locator-inference/cli-command/`; the gate records
    under `docs/planning/reproduce-locator-inference/match-and-propose/`.
