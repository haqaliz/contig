...
held-out-accuracy trend over corpus/loop versions. *(Superseded: `no_progress` and then
`qc_anomaly` are now covered — see below — so `covered_classes` is **7** over **9**
scenarios and **no failure class is structurally unreachable any more**. The remaining
uncovered classes have no scenario, not no mechanism.)*
