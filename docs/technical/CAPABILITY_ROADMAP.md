...
trace/executor fixtures. **Shipped (sibling-peak rescue slice — Unreleased):** the cut rung ships as the
fourth tier. When an OOM'd task's own peak is a `-`/0 (signal-killed), the retry
sizes from the **max positive peak among the same-coarse-`process` rows**, but
**only when that strictly exceeds the current limit** (new `current_gb` guard —
a sibling candidate that sizes at or below `current` would be a no-op retry, so
the blind `× 2` fires instead): new `PeakSizing` tiers `sibling_peak` /
`sibling_dominated` with `source_name` naming the borrowed row; the
`RepairStep.detail` strings carry the tier, the sibling's value + task name, and
the fallback reason. The prerequisite was the parser: both trace parsers now
read the coarse `process` column by header name with a `name or ""` fallback
(previously `process == name` for every row, which is why the rung was cut — the
`progress.py` "blast radius" was real but cosmetic: display-only). Honest scope
in the parent slice's voice: a surviving sibling's peak is bounded by the failed
limit, so the rung can never beat blind — precision, not recovery, stated as a
hypothesis; the shipped pin `test_same_process_sibling_is_not_rescued` flips to
`test_same_process_sibling_rescues_when_strictly_larger` (disclosed like the
`qc_anomaly` `recovered` flip); and the committed **revisit trigger** stands —
0 of the next 20 real OOM heals (counted from `RepairStep.detail` tier tokens)
restates the rung as taxonomy-only. Guards unmoved (the `oom` heal scenario's
trace has no peak column → blind path). Still deferred here: folding the
observed peak into the `FailureCase` corpus schema (telemetry rides in the
signed `RepairStep.detail` for now).
...
the sibling-`realtime` rescue is **subsumed, not blocked** — unlike the memory
sizer, `realtime_informed_time_h` already takes the **max realtime across all usage
rows** (`resource_sizing.py:114`), so it never shared the memory branch's
`process == name` blocker; the roadmap's earlier "same blocker as memory" claim
was inaccurate as written and is corrected here, with the time branch untouched.
Factor/ceiling calibration on real data stays deferred.
...
**Shipped (RO-Crate projection, Unreleased).** `contig export <run> --rocrate` now projects
`reference_identity` and `annotation_identity` into the crate: a `#reference` Dataset (an
iGenomes genome-key `PropertyValue`, or explicit FASTA/GTF `File`s with harmonization noted
via `additionalProperty`, plus one `File` per known-sites resource already on the record) and
one `#annotation-{n}` `SoftwareApplication` per annotation-provenance entry, linked from the
root via `mentions` with `#`-prefixed ids. A `@context` term map (`sha256`, `localPath`) fixes
a pre-existing defect where the RO-Crate 1.1 context left `sha256` unmapped, dropping every
checksum on every crate under JSON-LD expansion (applied to every crate, so **every exported
crate's `@context` bytes change**), while the `@graph` is unchanged for a record with no
reference and no annotation identity. `encodingFormat` is `{"@id": <EDAM IRI>}` linked to a
deduplicated `WebSite` entity per format, not a bare IRI string (RO-Crate 1.1 rule 27.1).
Validated with `roc-validator` 0.11.4 against profile `ro-crate-1.1` at `RECOMMENDED` severity
in a throwaway venv (not a project dependency): the term map fixes master's REQUIRED
"`sha256` not present in the `@context`" error, the new entities add zero new REQUIRED issues,
and one RECOMMENDED warning is kept deliberately (a one-item `mentions` stays a list); master's
pre-existing REQUIRED issues (no `license`/`datePublished`/`description`, unmapped
`parameters`/`verdict`/`qcResults`) are unchanged and out of scope. Roadmap-pull, not
demand-pull: nothing parses the crate today. No change to `models.py`, signing, the CLI or the
dashboard; stdlib only. The pre-flight **mismatch detector** (including known-sites/GTF-version
consistency) and **annotation/GTF version** resolution above remain deferred; this slice only
ships what the record already carries.
...
user-supplied `--vep_cache`/`--snpeff_cache` paths still a future slice. *(That future slice has
since shipped — `annotation-cache-inputs` (Unreleased): typed `--vep-cache`/`--snpeff-cache`
flags replayed by rerun/resume, an explicit-reference pre-flight refusal (both caches or
neither; the wrong-build download is never launched), user-cache suppression of the
auto-download wiring, and a cache-inputs provenance echo on the methods/HTML/text surfaces
from `RunRecord.parameters` — no model change, no signature break. iGenomes-mode safety is
assumed (per-genome attrs), with the manual smoke gate's GRCh37 run as the check.)* Test-first; no real
...
**Shipped (locator-inference candidate sweep — aspect 1 of 3, Unreleased).** `contig
extract-claims` emits a deliberately **locator-less** draft, so a human has had to open the
repo and hand-write a locator per claim before `contig reproduce` could bind anything — the
last manual link in the paper → verdict chain. This slice ships the **pure substrate** that
closes it: `verification/locator_inference.py` walks a repo and enumerates every numeric value
in its JSON/TSV/CSV/`.tab` artifacts as a **coordinate the shipped resolvers re-resolve**
(`sweep_repo` over `iter_artifacts` + `_json_candidates` + `_table_candidates`). It does **no
matching**, has **no CLI**, and changes no verdict, exit code, bundle, or signed field; a later
aspect matches candidates against a paper's stated values to *propose* a locator.

**It does not overturn the objection the extraction slice recorded.** That slice stated
locator-lessness was principled — "the paper gives the *value*, not *where it lives in the
repo's output*, so inventing a `from`/`path` locator would be dishonest." Correct as stated,
and this slice sidesteps rather than contradicts it: we never infer from the paper (that *is*
invention), we search the repo's **actual artifacts** and emit nothing unless the evidence is
unambiguous. **Evidence-gated search, not guessing.**

**The freshness guard cannot be weakened by this, structurally.** `_require_fresh` is a closure
nested inside `run_reproduction` (`reproduce.py:890`) — not importable, all five call sites
inside that one function. A pre-run, out-of-engine sweep cannot reach it. Inference proposes a
*binding site*, never evidence; a locator becomes a verdict only when a fresh run rebinds it
under the wholly unchanged guard.

**Honest scope.** **Push, not demand-pull.** It **recovers nothing for a user yet** — without
the matcher it proposes no locator. Correctness is **self-graded** (we authored every fixture),
and the honest remedy is the PRD's **evidence gate**: point the sweep plus the matcher at one
real published repo, count how many claims genuinely bind, and only then decide whether the CLI
aspect ships as designed, ships narrowed, or does not ship. A real-repo run therefore remains a
**manual gate**, per every prior C8 slice. **Deferred, with the blocker named:** `pattern` and
notebook inference need a *synthesized* regex, which collides with `resolve_match`'s strict
exactly-one rule (`reproduce.py:408`) — a technical blocker, not a preference. Plan/PRD under
`docs/planning/reproduce-locator-inference/`.

**Shipped (match-and-propose — aspect 2 of 3, Unreleased).** The pure matcher that closes the
"hand-write a locator per claim" link: `verification/locator_match.py` (`match_claims` /
`MatchOutcome` / `sidecar_text`) consumes the sweep's `Candidate`s and an `extract-claims`
draft, binding only on unambiguous evidence — rounding-aware equality at the claim's own
repr-derived printed precision (integer-valued claims compare exactly, so `875.6` never
matches `876`), strict exactly-one at **site** granularity (a distinct (source, coordinate)
pair; duplicated table rows are one site), dual-scale `raw`/`÷100` with the dead "both
scales, same site" branch collapsed by design, M9 low-information refusal (deny-list `0`/
`1`/`0.5`/`0.05`/`100` + integers with ≤ 2 significant digits), never-raises; locators
carried verbatim from candidates (never synthesized paths; complete table locators, no
`delimiter`); universal `load_claims` round-trip (M6), `classify`/`claim_family` parity
(G4), and a stale-artifact honesty pin proving inference cannot weaken the freshness guard
(`_require_fresh` is a closure inside `run_reproduction`, structurally unreachable from
inference). **The PRD evidence gate ran it on a real repo and measured the worst case: 0/20
binds** on `ritvikK05/rnaseq-reanalysis-htt` (full published result tables — every plausible
value appears at scale) — R2 measured, not assumed, and the gate did its job before any CLI
work. **The narrowed design (M10) ships:** `match_claims(..., *, metrics)` filters the pool
by the claim's metric words against table column headers / JSON leaf keys (normalized
equality or containment; fallback byte-identical value-only); `MatchOutcome.semantic_match`
and sidecar wording name the semantic path. **Second gate (shipped module, same repo):
6/20 bound, 0 wrong** — every bind re-resolved to exactly the claim value; expectation-vs-
actual disclosed (68.0 refused by M9 by design); two vocabulary-sensitivity cases measured
("called by both" vs `n_recovered`; "padj" vs `p.adjust`), both human-review-shaped.
**Shipped (`contig infer-locators` — aspect 3 of 3, Unreleased).** The command surface:
`infer-locators <repo> <claims.json> --out <out.json> [--metrics <json>] [--force]
[--dry-run]` — local-only (shipped `classify_repo_argument`, honest URL/DOI refusals),
extract-claims guard parity (`--out == input` clobber, `--force`, parent dir), the unchanged
`load_claims` round-trip before any atomic `os.replace` (temp via `mkstemp` in the out
parent), bound claims gain locator keys, non-bound claims semantically identical, review
sidecar = matcher lines + "Sweep skips (N)" + mode note, `--dry-run` writes nothing;
`--metrics` threads the M10 words (found in the dig: metric words do not survive to disk, and
`extract-claims` is untouched by design), optional with honest value-only fallback and
matched/unmatched-id counts in the echo so a stale map is never silent. **Manual smoke
(recorded):** the real command on the gate repo reproduces the gate's numbers end to end
(6/7/3/4, 0 wrong). Honest scope in the C8 voice throughout: push not demand-pull, one gate
repo at a moving `main`, self-graded fixtures, no real run in CI, no
`models.py`/verdict/bundle/signature change, no new dependency, guards unmoved. The paper →
verdict chain's last manual step is now review-only: `extract-claims` → review (metric
words) → `infer-locators` → review sidecar → `contig reproduce`. Records:
`docs/planning/reproduce-locator-inference/match-and-propose/` (spec amendment, plan, both
gate reports) and `cli-command/` (card, understanding, spec, plan).
...
| C5 | Reference and input-data integrity | M5 (reference-identity **capture** slice shipped — explicit `sha256` + iGenomes key-only, rendered in methods/panel; **RO-Crate projection of the identity SHIPPED (Unreleased)**; pre-flight **mismatch detector** (including known-sites/GTF-version consistency) and annotation/GTF version resolution pending) | Kills a silent-failure class, deepens reproduce |
