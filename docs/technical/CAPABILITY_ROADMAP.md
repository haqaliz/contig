...
dashboard; stdlib only. The pre-flight **mismatch detector**, real **known-sites capture**
(the model field exists but nothing populates it from a live run yet) and **annotation/GTF
version** resolution above remain deferred; this slice only ships what the record already
carries.
