...
`UNVERIFIED`) that **has not been run yet**; `--rev` is deferred so **nothing in the product
consumes the pin** — only a human can act on it (`git checkout <sha>`), making a fetched run
**auditable, not automatically replayable**, and `--depth 1` means the checkout is HEAD-at-fetch-
time rather than a revision the user chose; and **bundle-local checkouts are never pruned**, so the
...
saying so), `--rev`/tag/branch selection, hashing or signing the checkout tree, private-repo
credentials, submodules, and checkout pruning.
