...
def _review_sidecar(claims: list[ExtractedClaim], source: str | None = None) -> str:
    ...
    deterministic core. When `source` is set (the DOI as fetched, or the local
    PDF path), a `Source:` line is added to the header; when None the output is
    byte-identical to the text-path sidecar.
    """
    lines: list[str] = [
...
def extract_claims(
    paper: str = typer.Argument(
        ..., help="Path to a local paper text file (.txt/.md), or a .pdf, or a DOI."
    ),
    ...
    allow_fetch: bool = typer.Option(
        False,
        "--allow-fetch/--no-allow-fetch",
        help="Fetch a DOI's paper PDF over the network (a DOI is refused without it).",
    ),
) -> None:
    """Extract candidate numeric claims from a paper's text into a draft.

    Reads a paper's text (a local `.txt`/`.md`, a local `.pdf` via pdftotext, or
    a DOI's PDF fetched over the network when `--allow-fetch` is passed), runs
    the deterministic extractor (optionally augmented by the LLM assist unless
    `--no-llm`), and writes two files: a **locator-less** draft claims JSON at
    `--out` (`id`/`value`/`tolerance` only) and a `<out>.review.md` sidecar
    carrying all provenance and the reviewer's to-do list. This command never
    runs a pipeline, never touches the verdict/bundle path, and never emits a
    draft the reproduce loader would reject: the draft is validated by a
    round-trip through `load_claims` before it is committed to `--out`.

    Nothing is written on any input failure (missing/oversized/non-UTF-8 paper,
    a PDF over the size cap, a refused DOI, a failed fetch/extract, or an
    existing `--out` without `--force`). Add a locator to each claim (see the
    sidecar) before running `contig reproduce`.
    """
    paper_argument = classify_paper_argument(paper)
    if paper_argument.refusal is not None:
        typer.echo(paper_argument.refusal, err=True)
        raise typer.Exit(code=1)
    kind = paper_argument.kind
    doi = paper_argument.doi

    # A DOI reaches the network, so it is opt-in: without --allow-fetch the
    # flag mistake reports the flag mistake (the --rev-precedence precedent),
    # before anything is touched.
    if kind == "doi" and not allow_fetch:
        typer.echo(
            "a DOI paper requires --allow-fetch to fetch its PDF over the "
            "network; pass --allow-fetch, or download the PDF and pass its "
            "path instead",
            err=True,
        )
        raise typer.Exit(code=1)
    ...
    # Skipped for a DOI -- a DOI is not a path, and resolving it as one is
    # nonsense -- so a DOI never trips this check.
    if kind in ("text", "pdf"):
        try:
    ...
    # -- and checked before any fetch, so a DOI run never pays for the network
    # only to be refused (the remote-intake ordering precedent).
    ...
    if kind == "text":
        ...
        # pdf, or a DOI fetched to a temp PDF: stat-before-read with the
        # MAX_PDF_BYTES cap, then the extractor seam. A fetched temp is
        # removed on every path (success and refusal alike).
        tmp_name: str | None = None
        try:
            if kind == "doi":
                with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as handle:
                    tmp_name = handle.name
                code, message = default_paper_fetcher(doi, Path(tmp_name))
                if code != 0:
            ...
            if not pdf_path.is_file():
                ...
                raise typer.Exit(code=1)
            if stat_result.st_size > MAX_PDF_BYTES:
                typer.echo(
                    f"paper is {stat_result.st_size} bytes, over the "
                    f"{MAX_PDF_BYTES}-byte cap",
                    err=True,
                )
                raise typer.Exit(code=1)

            code, message = default_pdf_text_extractor(pdf_path)
            if code != 0:
                typer.echo(message, err=True)
                raise typer.Exit(code=1)
            text = message
            if len(text.encode("utf-8")) > _MAX_MATCH_BYTES:
                typer.echo(
                    f"extracted text is {len(text.encode('utf-8'))} bytes, over "
                    f"the {_MAX_MATCH_BYTES}-byte cap",
                    err=True,
                )
                raise typer.Exit(code=1)
        finally:
    ...
    # Sidecar provenance: the DOI as fetched, or the local PDF path, for the
    # pdf/doi kinds; None keeps the text path's sidecar byte-identical.
    ...
    if claims:
        sidecar_path.write_text(_review_sidecar(claims, source=source), encoding="utf-8")
    else:
        note = "# Draft claims -- review before `contig reproduce`\n\n"
        if source is not None:
            note += f"Source: {source}\n\n"
        note += "No numeric claims found; add claims by hand or check the input.\n"
        sidecar_path.write_text(note, encoding="utf-8")
...
def heal_guard(
    ...
    - **Reproduce-local, structurally outside this loop (1):**
      ...
      cannot reach it however many scenarios are added.
